...
const SIGN_PUBLIC_KEY_BYTES: usize = sodium::crypto_sign_PUBLICKEYBYTES as usize;
const SIGN_SECRET_KEY_BYTES: usize = sodium::crypto_sign_SECRETKEYBYTES as usize;
const SIGNATURE_BYTES: usize = sodium::crypto_sign_BYTES as usize;
...
impl fmt::Display for SodiumError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SignatureFailed => f.write_str("Ed25519 signature operation failed"),
        }
    }
}
...
pub fn random_public_bytes<const N: usize>() -> Result<[u8; N], SodiumError> {
    ...
    if N != 0 {
        // SAFETY: output is a live writable array of N bytes and libsodium has initialized.
        unsafe { sodium::randombytes_buf(output.as_mut_ptr().cast(), output.len()) };
    }
    Ok(output)
}

pub struct Ed25519SigningKeyPair {
    public_key: [u8; SIGN_PUBLIC_KEY_BYTES],
    secret_key: Mutex<SecureBytes>,
}

impl Ed25519SigningKeyPair {
    pub fn generate() -> Result<Self, SodiumError> {
        ensure_sodium()?;
        let mut public_key = [0_u8; SIGN_PUBLIC_KEY_BYTES];
        let mut secret_key = SecureBytes::allocate(SIGN_SECRET_KEY_BYTES)?;
        let status = secret_key.with_write(|secret_bytes| {
            // SAFETY: public_key and secret_bytes are writable buffers with exactly the sizes
            // required by crypto_sign_keypair and libsodium has initialized.
            unsafe {
                sodium::crypto_sign_keypair(public_key.as_mut_ptr(), secret_bytes.as_mut_ptr())
            }
        })?;
        if status != 0 {
            return Err(SodiumError::SignatureFailed);
        }
        Ok(Self {
            public_key,
    ...
    #[must_use]
    pub const fn public_key(&self) -> &[u8; SIGN_PUBLIC_KEY_BYTES] {
        &self.public_key
    }

    pub fn sign(&self, message: &[u8]) -> Result<[u8; SIGNATURE_BYTES], SodiumError> {
        ensure_sodium()?;
        ...
        if status != 0 || signature_len as usize != SIGNATURE_BYTES {
            signature.fill(0);
            return Err(SodiumError::SignatureFailed);
        }
        Ok(signature)
...
impl fmt::Debug for Ed25519SigningKeyPair {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("Ed25519SigningKeyPair")
...
mod tests {
    ...
    #[test]
    fn public_random_bytes_are_nonzero_with_overwhelming_probability() {
        let bytes = random_public_bytes::<32>().expect("random public bytes");
        assert!(bytes.iter().any(|byte| *byte != 0));
    }

    #[test]
    fn ephemeral_signing_key_never_exposes_secret_and_signs() {
        let key = Ed25519SigningKeyPair::generate().expect("signing key");
        let signature = key.sign(b"ENIGMA_PAIRING_BOOTSTRAP_V1").expect("signature");
        assert_eq!(key.public_key().len(), SIGN_PUBLIC_KEY_BYTES);
        assert_eq!(signature.len(), SIGNATURE_BYTES);
        assert!(!format!("{key:?}").contains("secret_key: ["));
    }
