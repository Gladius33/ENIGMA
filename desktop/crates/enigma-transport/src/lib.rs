...
pub struct RemoteDeviceKeyBundle {
    ...
    pub protocol_device_id: Option<u32>,
...
struct ClaimPreKeyResponse {
    user_id: String,
    device: RemoteDeviceKeyBundle,
}

#[derive(Debug, serde::Serialize)]
struct SendRelayMessageRequest<'a> {
    bubble_id: &'a str,
    ...
    message_type: &'a str,
    ciphertext: &'a str,
    attachment_blob_ids: &'a [String],
}

#[derive(Clone, Debug, serde::Deserialize, Eq, PartialEq)]
pub struct SentRelayMessage {
    pub id: String,
...
impl PairingRendezvousClient {
    ...
    pub fn current_account(
        &self,
        access_token: &[u8],
    ) -> Result<(String, Vec<LinkedDevice>), PairingRendezvousError> {
        let endpoint = self
            .base_url
            .join("v1/devices")
            .map_err(|_| PairingRendezvousError::InvalidEndpoint)?;
        ...
        let body = response
            ...
            .map_err(|_| PairingRendezvousError::InvalidResponse)?;
        if !is_canonical_uuid(&body.user_id)
            || body.devices.len() > 256
            || body.devices.iter().any(|device| {
                !is_canonical_uuid(&device.id)
                    || device.display_name.is_empty()
                    || device.display_name.len() > 256
                    || device.platform.is_empty()
                    || device.platform.len() > 64
            })
        {
            return Err(PairingRendezvousError::InvalidResponse);
        }
        Ok((body.user_id, body.devices))
    }
    ...
    pub fn discover_keys(
        ...
        let body = response
            .json::<KeyDiscoveryResponse>()
            .map_err(|_| PairingRendezvousError::InvalidResponse)?;
        if body.user_id != user_id || !valid_remote_bundles(&body.devices, false) {
            return Err(PairingRendezvousError::InvalidResponse);
        }
        Ok(body.devices)
    }
    ...
    pub fn claim_prekey(
        ...
        user_id: &str,
        device_id: &str,
    ) -> Result<RemoteDeviceKeyBundle, PairingRendezvousError> {
        if !is_canonical_uuid(user_id) || !is_canonical_uuid(device_id) {
            return Err(PairingRendezvousError::InvalidResponse);
        }
        let endpoint = self
        ...
        if body.user_id != user_id
            ...
            || !valid_remote_bundles(std::slice::from_ref(&body.device), true)
        {
            return Err(PairingRendezvousError::InvalidResponse);
        }
        Ok(body.device)
    ...
    pub fn send_relay_message(
        ...
        message_type: &str,
        ciphertext: &str,
    ) -> Result<SentRelayMessage, PairingRendezvousError> {
        if !is_canonical_uuid(bubble_id)
            || !is_canonical_uuid(sender_device_id)
            || !is_canonical_uuid(recipient_device_id)
            || !is_canonical_uuid(client_message_id)
            || sender_device_id == recipient_device_id
            || ciphertext.is_empty()
            || ciphertext.len() > 4 * 1024 * 1024
            || !matches!(message_type, "text" | "file" | "opaque")
        {
            return Err(PairingRendezvousError::InvalidResponse);
        }
        let endpoint = self
...
fn valid_remote_bundles(devices: &[RemoteDeviceKeyBundle], require_one_time: bool) -> bool {
    if devices.is_empty() || devices.len() > 256 {
        return false;
    }
    devices.iter().all(|device| {
        is_canonical_uuid(&device.device_id)
            && matches!(device.registration_id, Some(1..=16_380))
            && matches!(device.protocol_device_id, Some(1..=127))
            && device.identity_key.len() >= 16
            && device.identity_key.len() <= 8192
            && device.signed_prekey.key_id <= i32::MAX as u32
            && device.signed_prekey.public_key.len() >= 16
            && device.signed_prekey.signature.len() >= 16
            && device.kyber_prekey.as_ref().is_some_and(|key| {
                key.key_id <= i32::MAX as u32
                    && key.public_key.len() >= 16
                    && key.signature.len() >= 16
            })
            && (!require_one_time
                || device.one_time_prekey.as_ref().is_some_and(|key| {
                    key.key_id <= i32::MAX as u32 && key.public_key.len() >= 16
                }))
    })
}
